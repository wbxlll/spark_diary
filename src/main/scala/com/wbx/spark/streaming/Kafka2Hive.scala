    ...
    kfStream.foreachRDD(
      rdd => {
        ...
        if(!rdd.isEmpty()){
          ...
          val dataDf: DataFrame = sparkSession.createDataFrame(rowRDD, callHistorySchema()).repartition(1)
          dataDf.show()
          dataDf.write.mode(SaveMode.Append).insertInto("test.call_history")
        }
        //手动提交offset
