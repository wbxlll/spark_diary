...
object SparkWriteHiveOrcUsingTempView {

  def main(args: Array[String]): Unit = {
    //spark设置
    System.setProperty("HADOOP_USER_NAME", "root")
    val conf = new SparkConf().setAppName("SparkWriteHiveOrc").setMaster("local[1]")
    ...
    df.createOrReplaceTempView("tmp_call_history")
    val insertSQL = "insert into test.call_history_orc " +
      "select * from tmp_call_history"
    spark.sql(insertSQL)
